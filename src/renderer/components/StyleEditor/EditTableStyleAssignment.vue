    ...
    <v-card-text>
      <v-list>
        <v-list-item-group v-model="model" color="primary">
          ...
          <v-list-item
            v-if="assignment.geometryType === 1"
            v-for="icon in icons"
            :key="'icon' + icon.id"
            link
          >
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions({
        setTableStyle: 'Projects/setTableStyle',
      ...
      save () {
        if (this.model !== null && this.model !== undefined && this.model >= 0) {
          ...
          if (selection.styleRow) {
            this.setTableStyle({projectId: this.projectId, id: this.id, tableName: this.tableName, geometryType: this.assignment.geometryType, styleId: selection.styleRow.id, isGeoPackage: this.isGeoPackage})
            this.setTableIcon({projectId: this.projectId, id: this.id, tableName: this.tableName, geometryType: this.assignment.geometryType, iconId: -1, isGeoPackage: this.isGeoPackage})
          } else if (selection.iconRow) {
            this.setTableIcon({projectId: this.projectId, id: this.id, tableName: this.tableName, geometryType: this.assignment.geometryType, iconId: selection.iconRow.id, isGeoPackage: this.isGeoPackage})
            this.setTableStyle({projectId: this.projectId, id: this.id, tableName: this.tableName, geometryType: this.assignment.geometryType, styleId: -1, isGeoPackage: this.isGeoPackage})
          }
        } else {
          this.setTableStyle({projectId: this.projectId, id: this.id, tableName: this.tableName, geometryType: this.assignment.geometryType, styleId: -1, isGeoPackage: this.isGeoPackage})
          this.setTableIcon({projectId: this.projectId, id: this.id, tableName: this.tableName, geometryType: this.assignment.geometryType, iconId: -1, isGeoPackage: this.isGeoPackage})

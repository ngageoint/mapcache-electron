      ...
      async getServiceInfo (serviceType) {
        ...
        try {
          if (!_.isNil(this.dataSourceUrl) && !_.isEmpty(this.dataSourceUrl) && !_.isNil(serviceType) && serviceType !== -1) {
            if (serviceType === 2) {
              let request = superagent.get(this.dataSourceUrl.replace('{z}', '0').replace('{x}', '0').replace('{y}', '0'))
              ...
              for (let i = 0; i < supportedWMSVersions.length; i++) {
                ...
                let request = superagent.get(GeoServiceUtilities.getGetCapabilitiesURL(this.dataSourceUrl, version, 'WMS'))
                request.set('User-Agent', remote.getCurrentWebContents().session.getUserAgent())
                let credentials = this.getCredentials()
                if (credentials && (credentials.type === 'basic' || credentials.type === 'bearer')) {
                  request.set('Authorization', credentials.authorization)
              ...
              for (let i = 0; i < supportedWFSVersions.length; i++) {
                ...
                let request = superagent.get(GeoServiceUtilities.getGetCapabilitiesURL(this.dataSourceUrl, version, 'WFS'))
                request.set('User-Agent', remote.getCurrentWebContents().session.getUserAgent())
                let credentials = this.getCredentials()
                if (credentials && (credentials.type === 'basic' || credentials.type === 'bearer')) {
                  request.set('Authorization', credentials.authorization)

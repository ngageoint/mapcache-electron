...
export default class GeoTiffRenderingUtilities {
  ...
  static requestTile (tileRequest) {
    return new Promise ((resolve, reject) => {
      ...
      try {
        canvas = CanvasUtilities.createCanvas(width, height)
        const ctx = canvas.getContext('2d')
        const imageData = CanvasUtilities.makeImageData(width, height)
          ...
          if (intersection != null) {
            ...
            if (renderingMethod === 0) {
              ...
              if (grayBand > 0) {
                ...
                let setG = grayScaleColorGradient === 0 ? (
                  stretchToMinMax ? (position, sample) => {
                    const value = maxByteValue - GeoTiffUtilities.stretchValue(g(sample), grayBandMin, grayBandMax)
                  ...
                  stretchToMinMax ? (position, sample) => {
                    const value = GeoTiffUtilities.stretchValue(g(sample), grayBandMin, grayBandMax)
                    targetData[position] = value
                    targetData[position + 1] = value
                    targetData[position + 2] = value
                  } : (position, sample) => {
              ...
              if (redBand > 0) {
                ...
                setR = stretchToMinMax ? (position, sample) => targetData[position] = GeoTiffUtilities.stretchValue(r(sample), redBandMin, redBandMax) : (position, sample) => targetData[position] = r(sample) / redBandDataTypeMax * maxByteValue
              }
              if (greenBand > 0) {
                greenBandDataTypeMax = GeoTiffUtilities.getMaxForDataType(bitsPerSample[greenBand - 1])
                g = (sample) => sample[greenBand - 1]
                setG = stretchToMinMax ? (position, sample) => targetData[position + 1] = GeoTiffUtilities.stretchValue(g(sample), greenBandMin, greenBandMax) : (position, sample) => targetData[position + 1] = g(sample) / greenBandDataTypeMax * maxByteValue
              }
              if (blueBand > 0) {
                blueBandDataTypeMax = GeoTiffUtilities.getMaxForDataType(bitsPerSample[blueBand - 1])
                b = (sample) => sample[blueBand - 1]
                setB = stretchToMinMax ? (position, sample) => targetData[position + 2] = GeoTiffUtilities.stretchValue(b(sample), blueBandMin, blueBandMax) : (position, sample) => targetData[position + 2] = b(sample) / blueBandDataTypeMax * maxByteValue
              }
              if (alphaBand > 0) {
        ...
        CanvasUtilities.disposeCanvas(canvas)
        fs.close(fd)
      }
    })
  }

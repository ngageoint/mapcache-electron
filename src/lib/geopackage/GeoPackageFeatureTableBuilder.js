...
import GeoPackageFeatureTableUtilities from './GeoPackageFeatureTableUtilities'
import GeoPackageStyleUtilities from './GeoPackageStyleUtilities'
import GeoPackageMediaUtilities from './GeoPackageMediaUtilities'
 ...
 */
export default class GeoPackageFeatureTableBuilder {
  ...
  static async buildFeatureLayer (configuration, statusCallback) {
    return GeoPackageCommon.performSafeGeoPackageOperation(configuration.path, async (gp) => {
      ...
      for (let i = 0; i < configuration.sourceLayers.length; i++) {
        ...
        await GeoPackageCommon.performSafeGeoPackageOperation(sourceLayer.geopackageFilePath, async (geopackage) => {
          sourceFeatureMap[sourceIdx] = GeoPackageFeatureTableUtilities._getAllFeatureRowsIn4326(geopackage, sourceLayer.sourceLayerName, 4326, boundingBoxFilter)
          sourceColumnMap[sourceIdx] = GeoPackageFeatureTableUtilities._getFeatureColumns(geopackage, sourceLayer.sourceLayerName)
          sourceStyleMap[sourceIdx] = {
            styles: GeoPackageStyleUtilities._getStyleRows(geopackage, sourceLayer.sourceLayerName),
            icons: GeoPackageStyleUtilities._getIconRows(geopackage, sourceLayer.sourceLayerName),
            featureStyleMapping: GeoPackageStyleUtilities._getFeatureStyleMapping(geopackage, sourceLayer.sourceLayerName, numberLayersToRetrieve > 1),
            tableStyleMappings: GeoPackageStyleUtilities._getTableStyleMappings(geopackage, sourceLayer.sourceLayerName),
          ...
          const result = GeoPackageFeatureTableUtilities.mergeFeatureColumns(featureColumns, GeoPackageFeatureTableUtilities._getFeatureColumns(geopackage, sourceLayer.sourceLayerName))
      ...
      for (let i = 0; i < configuration.geopackageLayers.length; i++) {
        ...
        await GeoPackageCommon.performSafeGeoPackageOperation(geopackageLayer.geopackage.path, async (geopackage) => {
          sourceFeatureMap[sourceIdx] = GeoPackageFeatureTableUtilities._getAllFeatureRowsIn4326(geopackage, geopackageLayer.table, 4326, boundingBoxFilter)
          sourceColumnMap[sourceIdx] = GeoPackageFeatureTableUtilities._getFeatureColumns(geopackage, geopackageLayer.table)
          sourceStyleMap[sourceIdx] = {
            styles: GeoPackageStyleUtilities._getStyleRows(geopackage, geopackageLayer.table),
            icons: GeoPackageStyleUtilities._getIconRows(geopackage, geopackageLayer.table),
            featureStyleMapping: GeoPackageStyleUtilities._getFeatureStyleMapping(geopackage, geopackageLayer.table, numberLayersToRetrieve > 1),
            tableStyleMappings: GeoPackageStyleUtilities._getTableStyleMappings(geopackage, geopackageLayer.table),
          ...
          const result = GeoPackageFeatureTableUtilities.mergeFeatureColumns(featureColumns, GeoPackageFeatureTableUtilities._getFeatureColumns(geopackage, geopackageLayer.table))
          featureColumns = result.mergedColumns
          sourceNameChanges[sourceIdx] = result.nameChanges
          sourceIdx++
          layersRetrieved++
      ...
      const featureCollection = {
        ...
        features: GeoPackageCommon.flatten(values(sourceFeatureMap)).map(row => {
          return {
            type: 'Feature',
            properties: {},
        ...
        for (let featureRowIndex = 0; featureRowIndex < featureRows.length; featureRowIndex++) {
          ...
          columns.getColumns().forEach(column => {
            ...
            if (isNil(value) && tableColumn.isNotNull() && !tableColumn.hasDefaultValue()) {
              value = GeoPackageCommon.getDefaultValueForDataType(tableColumn.getDataType())
            }
            values[name] = value
          })
          ...
          featureColumns.getColumns().forEach(column => {
            ...
            if (!isNil(values[columnName]) && column.isNotNull() && !column.hasDefaultValue) {
              values[columnName] = GeoPackageCommon.getDefaultValueForDataType(column.getDataType())
            }
          })
          ...
          for (let i = 0; i < mediaToAdd.length; i++) {
            ...
            if (isNil(insertedMediaMap[sourceMediaKey])) {
              // get media
              const sourceMediaRow = await GeoPackageMediaUtilities.getMediaRow(geopackageFilePath, sourceMediaTable, sourceMediaRowId)
              ...
              requiredColumns.forEach(columnName => {
                ...
                mediaRow.setValueWithColumnName(columnName, GeoPackageCommon.getDefaultValueForDataType(type))
              })
            ...
            GeoPackageFeatureTableUtilities._linkFeatureRowToMediaRow(gp, tableName, featureId, mediaTableName, insertedMediaMap[sourceMediaKey])
          }

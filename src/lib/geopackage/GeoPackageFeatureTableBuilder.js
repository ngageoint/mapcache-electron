    ...
    return performSafeGeoPackageOperation(configuration.path, async (gp) => {
      ...
      try {
        ...
        for (let i = 0; i < configuration.sourceLayers.length; i++) {
          ...
          await performSafeGeoPackageOperation(sourceLayer.geopackageFilePath, async (geopackage) => {
            sourceFeatureMap[sourceIdx] = _getAllFeatureRowsIn4326(geopackage, sourceLayer.sourceLayerName, 4326, boundingBoxFilter)
            sourceColumnMap[sourceIdx] = _getFeatureColumns(geopackage, sourceLayer.sourceLayerName)
            sourceStyleMap[sourceIdx] = {
              styles: _getStyleRowObjects(geopackage, sourceLayer.sourceLayerName),
              icons: _getIconRowObjects(geopackage, sourceLayer.sourceLayerName),
              featureStyleMapping: _getFeatureStyleMapping(geopackage, sourceLayer.sourceLayerName, numberLayersToRetrieve > 1),
              tableStyleMappings: _getTableStyleMappings(geopackage, sourceLayer.sourceLayerName),
            ...
            const result = mergeFeatureColumns(featureColumns, _getFeatureColumns(geopackage, sourceLayer.sourceLayerName))
        ...
        for (let i = 0; i < configuration.geopackageLayers.length; i++) {
          ...
          await performSafeGeoPackageOperation(geopackageLayer.geopackage.path, async (geopackage) => {
            sourceFeatureMap[sourceIdx] = _getAllFeatureRowsIn4326(geopackage, geopackageLayer.table, 4326, boundingBoxFilter)
            sourceColumnMap[sourceIdx] = _getFeatureColumns(geopackage, geopackageLayer.table)
            sourceStyleMap[sourceIdx] = {
              styles: _getStyleRowObjects(geopackage, geopackageLayer.table),
              icons: _getIconRowObjects(geopackage, geopackageLayer.table),
              featureStyleMapping: _getFeatureStyleMapping(geopackage, geopackageLayer.table, numberLayersToRetrieve > 1),
              tableStyleMappings: _getTableStyleMappings(geopackage, geopackageLayer.table),
            ...
            const result = mergeFeatureColumns(featureColumns, _getFeatureColumns(geopackage, geopackageLayer.table))
            featureColumns = result.mergedColumns
            sourceNameChanges[sourceIdx] = result.nameChanges
            sourceIdx++
            layersRetrieved++

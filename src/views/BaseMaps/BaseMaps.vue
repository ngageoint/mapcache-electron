    ...
    <v-sheet class="mapcache-sheet-content mapcache-fab-spacer detail-bg">
      <v-list class="pa-0">
        <template v-for="item in baseMapItems" :key="item.id + '-base-map'">
          <v-list-item @click="() => showBaseMap(item.id)" class="mt-1 mb-1">
      ...
      <template v-slot:activator="{ props }">
        <div class="fab-position" v-bind="props">
          <v-btn
              ...
              color="primary">
            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24" width="24" height="24">
              <defs>
                <path
      ...
      baseMapItems: state => {
        return getDefaultBaseMaps().concat(state.BaseMaps.baseMaps || []).map(baseMap => {
          ...
          return {
            ...
            missingRaster: baseMap.layerConfiguration ? window.mapcache.isRasterMissing(baseMap.layerConfiguration.layerType, baseMap.layerConfiguration.rasterFile) : false,

<template>
  <v-card>
    <v-card-title>
      {{title}}
      <v-spacer/>
      <geometry-style-svg v-if="geometryType === 1" :geometry-type="1" :color="color" :fill-color="fillColor" :fill-opacity="fillOpacity"/>
      <geometry-style-svg v-if="geometryType === 2" :geometry-type="2" :color="color" :fill-color="fillColor" :fill-opacity="fillOpacity"/>
      <geometry-style-svg v-if="geometryType === 3" :geometry-type="3" :color="color" :fill-color="fillColor" :fill-opacity="fillOpacity"/>
    </v-card-title>
    <v-card-text>
      <v-row no-gutters>
        <v-col cols="5">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      saveStyle () {
        this.save({color: this.color, opacity: this.opacity, fillColor: this.fillColor, fillOpacity: this.fillOpacity, width: this.width})
      }
    }
  }
</script>
